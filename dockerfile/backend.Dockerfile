...
FROM python:3.9.0-slim AS server-composite-data
LABEL authors="SimpleIN1 <serbinovichgs@ict.nsc.ru>"
...
COPY --chown=django-user:django-user ./backend/CompositeDataRESTAPIService/requirements.txt .

RUN python -m venv venv && venv/bin/python -m pip install --upgrade pip
RUN --mount=type=cache,target=/root/.cache/pip \
    venv/bin/pip install -r requirements.txt

COPY --chown=django-user:django-user ./backend/CompositeDataRESTAPIService/api/ api
COPY --chown=django-user:django-user ./backend/CompositeDataRESTAPIService/migrations migrations
COPY --chown=django-user:django-user ./backend/CompositeDataRESTAPIService/alembic.ini .
COPY --chown=django-user:django-user ./backend/CompositeDataRESTAPIService/start.sh .
RUN chmod u+x /usr/src/app/start.sh
